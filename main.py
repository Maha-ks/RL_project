...
all_rewards = np.array(all_rewards)           # (n_seeds, n_episodes)
all_successes = np.array(all_successes)       # (n_seeds, n_episodes) 0/1
...
plt.fill_between(episodes, mean_rewards - std_rewards, mean_rewards + std_rewards, alpha=0.2, label="±1 std")
...
plt.fill_between(episodes, mean_success_rate - std_success_rate, mean_success_rate + std_success_rate, alpha=0.2, label="±1 std")
...
# 3) Unique states visited (mean ± std)
plt.figure()
plt.plot(episodes, sm_unique, label="Mean unique states (smoothed)")
plt.fill_between(episodes, mean_unique - std_unique, mean_unique + std_unique, alpha=0.2, label="±1 std")
...
# (Nice-to-have) Summary stats for the report
...
final_success_std = mean_success_rate[-last_n:].std()
